...
- 2026-08-17: Review pass (`/code-review` over the story's diff) returned four low-severity findings, all fixed in `7f4f8c2`. Three were the same shape — the 409 conflict path doing less than the 200 path: `_approval_conflict` re-reads the whole tab and so can raise `MissingStateRate`, but it runs inside `except StalePaymentRow` where the route's own handler cannot see it (a claim with no state rate schedule answered an unhandled 500 instead of the documented problem document); `useApprovePayment.onError` installed the fresh payload without invalidating the case file, leaving the treatment Overview card's disbursed-of-scheduled figures behind the Bills tab; and the sheet's refusal, keyed against following the handler to a *different* row, still greeted them on the way back into the same one. The fourth was latent: `_approve` returned the claim's business id as its `Approval.entity_id`, discarding the row identity ("week 3") that this story's own docstring names Story 3.5's checklist as the consumer of. Three regression tests added, each confirmed failing without its fix; the DB-backed suite was run against a real Postgres rather than skipped, so the gate is pytest 1539 (0 skipped), vitest 296, mypy/tsc/ruff/eslint clean. Accepted after human review — status → done.
