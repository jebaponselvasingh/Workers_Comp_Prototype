...
3. **Given** `audit_event`, **when** purge touches it, **then** rows are redacted in place — the skeleton (actor, action, entity, timestamps) survives while `before`/`after` are overwritten with a redaction marker — executed under the `audit_redactor` DB role whose only grants are SELECT (without which neither qualified statement below can run), UPDATE on those columns, and RLS-constrained DELETE beyond the retention floor (AD-4 exception).
...
- [ ] Task 1: `audit_redactor` DB role migration (AC: 3, 5)
  - [ ] Alembic migration creating the `audit_redactor` role with exactly three grants and nothing else: `GRANT SELECT ON audit_event` (required for the other two to work on a qualified statement — Postgres needs SELECT on every column named in a `WHERE`, and a matching SELECT *policy*, or a targeted redaction silently matches zero rows), `GRANT UPDATE (before, after) ON audit_event` and `GRANT DELETE ON audit_event` constrained by a row-level-security policy `at < now() - <retention floor>` (floor injected from config at policy-evaluation time — see Task 2 for the knob; do not hardcode `7 years` in the policy if the config can drive it, e.g. via a `current_setting()` GUC set by the purge session)
