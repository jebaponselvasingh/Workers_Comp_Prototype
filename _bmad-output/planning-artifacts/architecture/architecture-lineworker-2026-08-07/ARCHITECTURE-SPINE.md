...
### AD-16 — Untrusted-content discipline: injected text is data, never instructions

- **Binds:** every string entering model context from outside `agents/prompts/` — user chat messages, claim fields and narratives, diary notes, emails, document text, RAG-retrieved `knowledge_chunk` and similar-case content, tool `{ok, data, display}` payloads; the copilot approval UI; all model-output handling in `agents/` and `web/features/copilot`
- **Prevents:** prompt injection riding claim data or retrieved chunks into tool selection, routing, scope, or writes; the approval dialog describing a different action than the one that executes; model output being interpreted as markup or instructions downstream; a false sense of safety from unverifiable "injection detection"
- **Rule:** Exactly one instruction channel exists: the versioned prompt files in `agents/prompts/` (Prompts convention). Every other string that reaches model context is **untrusted data** — including content the org itself stored, since claim narratives, diary notes, and documents originate from injured workers, employers, and providers. **Delimiting:** tool results enter context only inside the `{ok, data, display}` envelope; retrieved chunks enter per-chunk delimited and tagged with their source id; every system prompt carries a standing instruction that delimited content is material to analyze, never commands to follow. **No data-derived control flow:** nothing parsed from untrusted content may select a tool, alter a prompt, name a scope, change a route, or trigger a refresh — routing comes only from the static QAS key→node map or the user's own free-text turn (AD-14), scope only from the registry-injected caller context (AD-7/AD-13), tool arguments only via their typed Pydantic schemas. **Containment floor (the actual safety case):** a fully hijacked model still cannot execute an unapproved write (AD-6 gate), read outside the caller's scope (AD-7/AD-13), or exfiltrate — inference is local-only with no cloud path (AD-5) and the agent holds no network egress tools. These invariants, not content filtering, are what the safety claim rests on. **Approval honesty:** the interrupt payload the approval UI renders is the middleware's actual pending tool call — tool name and typed arguments, server-supplied — so the human approves the payload that will execute, never the model's paraphrase of it. **Output discipline:** assistant prose renders as sanitized markdown only (never raw HTML, never executed); structured outputs validate against their Pydantic models before the UI sees them; URLs in model output are never auto-fetched by server or SPA. **Tests:** adversarial fixtures — injection strings seeded into a claim field, a diary note, and a knowledge chunk — run in the agent graph tests and the AD-15 e2e suite, asserting no unapproved write, no scope widening, and no route change. Content-level injection *detection*/filtering heuristics are deliberately out of scope (Deferred): the discipline bounds blast radius; it does not claim the model cannot be talked into bad prose — that residual risk is why AD-2 keeps figures deterministic and AD-6 keeps writes human-gated.
...
| Copilot chat, quick actions, RTW letter (FR-H-9/11, FR-CP-*) | `agents/` + `web/features/copilot` | AD-2, AD-5, AD-6, AD-7, AD-13, AD-14, AD-16 |
| Similar-case + labor-law RAG, AI-insight cache | `services/rag` (owns embeddings + `ai_insight`; sole embeddings client of Ollama) + `agents/` | AD-3, AD-5, AD-7, AD-10, AD-12, AD-13, AD-16 |
