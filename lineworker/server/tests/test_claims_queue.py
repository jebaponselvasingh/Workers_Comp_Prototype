...
async def test_both_rule_document_versions_that_ranked_the_queue_are_reported(
    ...
    # Three, not one, since Story 2.2 superseded the thresholds document with
    # a v2 carrying the treatment-phase parameters and Story 2.5 a v3 carrying
    # the claim-path ones. Pinned rather than read from the loader for
    # `seed_fixture`'s reason — and the number moving is the mechanism
    # working: a retuned threshold re-ranks the queue, so the payload has to
    # say which version answered.
...
async def test_a_second_priority_weights_version_reranks_the_queue(seeded_db_url: str) -> None:
    ...
    assert after["thresholdsVersion"] == 3, "only one document was superseded"
