...
take a caller context and apply one unconditional employer filter. Three
...
and app_user rows), so it necessarily runs before one exists; `glossary` and
`statutory_forms` read reference data that has no employer, no claim and no
PHI, so there is nothing for a filter to narrow.
...
from data.repositories import claims, glossary, identity, statutory_forms

__all__ = ["claims", "glossary", "identity", "statutory_forms"]
