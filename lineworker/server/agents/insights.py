...
# Re-exported as well as used. `tests/test_prompt_injection_fixtures.py` and
# every reader who learned this vocabulary here still find it here, so moving
# the two functions to `agents/fencing.py` (Story 6.3, to break a cycle with
# `agents/tools/claim.py`) cost no caller a change. `scrub` is not called
# directly by this module any more — `fence` calls it — and is named here so
# that `from agents.insights import scrub` keeps working.
from agents.fencing import (  # noqa: F401
    FIGURES_HEADING,
    ITEM_CLOSE,
    ITEM_OPEN,
    MATERIAL_HEADING,
    ...
    **One instance per claim per run.** The generator caches the claim's case
    ...
    async def _narrate[T: InsightModel](
        ...
        *,
        ...
        headings are module constants rather than literals here, so that `fence`
    ...
    async def _claim_narrative(self, claim_business_id: str) -> list[str]:
        ...
        return [
            fence(f"claim:{claim_business_id}:injury_type", header.injury_type),
            fence(f"claim:{claim_business_id}:cause", header.cause),
            fence(f"claim:{claim_business_id}:icd", f"{header.icd} {header.body_part}"),
        ]
    ...
    async def _knowledge(self, query: str) -> list[str]:
        ...
        **"Outage" is a narrow word here, and it has to be** (follow-up review
        ...
        try:
            ...
            return []
        return [fence(f"knowledge:{hit.source}", f"{hit.title}: {hit.chunk_text}") for hit in hits]
